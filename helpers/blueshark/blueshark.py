        ...
        try:
            ...
            if 1 <= choice <= len(ports):
                ...
                if "Sniffer" not in port.description:
                    print("[i] Please make sure that the nRF runs with the sniffer firmware (sniffer_nRF52840_4.1.1.zip).\nYou can use the sbleedy firmware installer by running `sbleedy -fhw nRF52840`.")
                    exit()
                return port.device
            elif choice == 0:
                exit()
...
def start_scan_hackRF(args):
    check_setup_hackRF()
    path = BLUESHARK_DIR + "/ice9-bluetooth-sniffer/build"
    command = ["./ice9-bluetooth", "-l"]

    if args.freq:
        command += ["-c", str(args.freq)]
    if args.channels:
    ...
    try:
        ...
        process = subprocess.Popen(command, cwd=path, stdout=sys.stdout, stderr=sys.stderr, text=True)
        ...
        if args.wireshark:
            ...
            pcap_file = currPath + "/logs/" + args.logfile + ".pcap"
            if os.path.exists(pcap_file):
                try:
                    print(f"Opening {pcap_file} in Wireshark")
                    subprocess.run(["wireshark", pcap_file], check=True)
...
def main():
    ...
    parser.add_argument('-f','--freq', required=False, type=str, help="center channel frequency in MHz, between 2402 and 2480 (required for hackRF)")
